	...
	http.get('http://localhost:1800/rhapsody/guild/getChannelId?g='+message.guild.id, (resp) => {
		...
		resp.on('end', () => {
			if (resp.statusCode == '200') {
				...
				if (data.status == 404) {
					message.reply('No voice channel to leave.');
					return;
				} else {
				...
				message.reply('An error occurred trying to get the resource.```status: ' +resp.statusCode+ '\nguildId: ' +message.guild.id+ '```');
			}
		});
	}).on('error', () => {
		message.reply('An error occurred trying to get the resource.');
	}); 
};
