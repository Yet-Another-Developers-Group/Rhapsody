...
const http = require('http');
 exports.run = (client, message, args) => ***REMOVED***
     ...
     ***REMOVED***
     http.get('http://localhost:1800/rhapsody/addNewGuildChannel?g='+ message.guild.id +'&c='+ VoiceChannel.id, (resp) => ***REMOVED***
     let data = '';
     ...
     resp.on('end', () => ***REMOVED***
          if (resp.statusCode == "200") ***REMOVED***
               ...
               ***REMOVED*** else ***REMOVED***
                    message.inlineReply('An error occurred trying to get the resource.```status: ' +data.status+ '\nvoiceChannelId: ' +VoiceChannel.id+ '\nguildId: ' +message.guild.id+ '```Try using `stop` and then using `join` again. If taht doesn\'t work, contact us.');
               ***REMOVED***
          ***REMOVED*** else ***REMOVED***
